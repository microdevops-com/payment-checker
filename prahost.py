...
def prahost(username, password, twofa_secret, item_number, proxy):

    with sync_playwright() as p:

        print("Checking PraHost with a browser for the username:", username)
        ...
        if page.inner_text("body").find("Your Info") != -1:
            ...
            page.click("button.button.button-block")
        ...
        page.goto("https://www.prahost.com/clientarea.php?action=invoices")
        page.wait_for_load_state("load")
        ...
        # The invoices data is already there, it is in that table
        # <table id="tableInvoicesList"
        # In that table, in <tbody>, walk through all <tr>
        # First <td> is the invoice number
        # Third <td> os the due date
        ...
        for tr in page.query_selector_all("table#tableInvoicesList tbody tr"):
            ...
            td3 = tr.query_selector("td:nth-child(3) span.hidden")
            td5 = tr.query_selector("td:nth-child(5) span.label.status")
            # Append to the list
